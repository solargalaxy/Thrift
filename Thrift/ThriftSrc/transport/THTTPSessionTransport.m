...
-(void) flushWithCompletion:(TAsyncCompletionBlock)completed failure:(TAsyncFailureBlock)failure
{
  ...
  NSURLSessionDataTask *task = [self.factory taskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
    ...
    if (!error) {
      error = [weakSelf.factory validateResponse:httpResponse data:data];
    }

    weakSelf.responseDataOffset = 0;
